...
export default function AdminRestockLogsClient() {
  ...
  const goPage = (p: number) => {
    ...
    // keep existing filters when paging (optional but recommended)
    const nextQs = new URLSearchParams(sp.toString());
    nextQs.set('page', String(next));
    router.push(`/admin/restocks?${nextQs.toString()}`);
  };
  ...
  return (
    <AppShell>
      ...
      {isLoading && <div className="py-10 text-sm text-neutral-600">Loading restock logs…</div>}
